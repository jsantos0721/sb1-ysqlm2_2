...
function RegistrationForm() {
  ...
  const renderFileInput = (name: string, label: string, helpText?: string) => (
    <div className="flex flex-col">
      ...
      <div className="relative">
        ...
        <div className="bg-red-500 text-white py-2 px-4 rounded-full text-sm font-semibold hover:bg-red-600 transition-colors duration-300 cursor-pointer">
          {fileData[name as keyof FileData] ? 'Archivo seleccionado' : 'Seleccionar archivo'}
        </div>
      </div>
    </div>
  );
      ...
      <div className="relative px-4 py-10 bg-white shadow-lg sm:rounded-3xl sm:p-20">
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex items-center">
              {isAdmin && (
                <button onClick={() => setShowAdminScreen(true)} className="mr-4 text-red-500 hover:text-red-700">
                  <Settings size={24} />
                </button>
              )}
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {renderFileInput("dniFile", "Adjuntar DNI")}
            <div className="flex flex-col">
              <label htmlFor="nss">Certificado de la Seguridad Social</label>
              <input type="text" id="nss" name="nss" value={formData.nss} onChange={handleChange} className="form-input mt-1" required />
            </div>
            {renderFileInput("nssFile", "Adjuntar Certificado SS", "Puede encontrar el certificado en la página https://sede.seg-social.gob.es/wps/portal/sede/sede/Ciudadanos/Informes+y+Certificados/201832")}
            <div className="flex items-center">
            ...
            {renderFileInput("titulacionFile", "Adjuntar Titulación")}
            {renderFileInput("cvFile", "Adjuntar CV")}
            {renderFileInput("prlFile", "Adjuntar Curso PRL")}
